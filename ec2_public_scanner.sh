# This tool scans an AWS account for public EC2s. It will then save the IPs to target.txt and then run an nmap scan against them for more info
#!/bin/bash

print_help() {
    echo "This is a tool that finds public EC2 instances in an AWS account and returns their public IPs. The public ones will be stored in target.txt
...
echo "EC2 Public IP Finder is a tool that finds public EC2 instances in an AWS account and returns thier public IPs. If they're public, it will be echoed to the screen in green writing as well as saved to 'targets.txt' for manual enumeration"
...
for instance in $(aws ec2 describe-instances --profile "$AWS_PROFILE" --region "$AWS_REGION" --filters "Name=instance-state-name,Values=running" --query "Reservations[].Instances[].InstanceId" --output text); do
    ...
    if [[ -n $public_ip" ]]; then
      echo -e "\033[23mInstance $instance has public IP $public_ip033[0m"
